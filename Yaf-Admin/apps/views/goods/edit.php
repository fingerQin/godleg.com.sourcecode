...
.spec-table .prods-area dl dt{padding-right:15px;text-align:right}
...
<div class="main">
    <form action="{{'Goods/edit'|url}}" method="post" name="myform" id="goods-prods-form">
        <table class="content" border="0" cellspacing="0" cellpadding="0">
            ...
            <tr>
                ...
                <td>
                    <select name="cat_id" id="cat_id" class="form-control">
                        ...
                        <option {{if $data.cat_code==$cat.cat_code}}selected="selected"{{/if}} value="{{$cat.cat_id}}">{{$cat.cat_name}}</option>
                        {{foreach $cat.sub as $cat_two}}
                        <option {{if $data.cat_code==$cat_two.cat_code}}selected="selected"{{/if}} value="{{$cat_two.cat_id}}">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;├─{{$cat_two.cat_name}}</option>
                        {{foreach $cat_two.sub as $cat_three}}
                        <option {{if $data.cat_code==$cat_three.cat_code}}selected="selected"{{/if}} value="{{$cat_three.cat_id}}">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;├─{{$cat_three.cat_name}}</option>
                        {{/foreach}}
                        {{/foreach}}
                        {{/foreach}}
                    </select>
            ...
            <tr>
                ...
                <td>
                    <select name="marketable" id="marketable" class="form-control" style="width:120px;">
                        <option {{if $data.marketable==0}}selected="selected"{{/if}} value="0">下架</option>
                        <option {{if $data.marketable==1}}selected="selected"{{/if}} value="1">上架</option>
                    </select>
                </td>
            </tr>
            <tr>
                <th class="left-txt">排序值(值小排前):</th>
                <td><input type="text" name="listorder" id="listorder" size="5" class="form-control" style="width:120px;" value="{{$data.listorder}}"></td>
            </tr>
            <tr>
                <th class="left-txt">商品相册：</th>
                <td>
                    <input type="hidden" name="goods_album[]" id="input_voucher1" value="{{$data.goods_image.0}}" />
                    <div id="previewImage1" class="goods_image">
                        <img width="120" height="120" src="{{$data.goods_image.0}}" />
                    </div>

                    <input type="hidden" name="goods_album[]" id="input_voucher2" value="{{$data.goods_image.1}}" />
                    <div id="previewImage2" class="goods_image">
                        <img width="120" height="120" src="{{$data.goods_image.1}}" />
                    </div>

                    <input type="hidden" name="goods_album[]" id="input_voucher3" value="{{$data.goods_image.2}}" />
                    <div id="previewImage3" class="goods_image">
                        <img width="120" height="120" src="{{$data.goods_image.2}}" />
                    </div>

                    <input type="hidden" name="goods_album[]" id="input_voucher4" value="{{$data.goods_image.3}}" />
                    <div id="previewImage4" class="goods_image">
                        <img width="120" height="120" src="{{$data.goods_image.3}}" />
                    </div>

                    <input type="hidden" name="goods_album[]" id="input_voucher5" value="{{$data.goods_image.4}}" />
                    <div id="previewImage5" class="goods_image">
                        <img width="120" height="120" src="{{$data.goods_image.4}}" />
                    </div>
                </td>
            </tr>
                ...
                <td>
                    <div class="spec-table" style="width:90%">
                        <div class="prods-area" id="spec-area">
                            ...
                            {{foreach $data.spec_val as $spec_name => $spec_val}}
                            <dl data-spec-key="{{$spec_name}}">
                                <dt>
                                    <img class="del_spec_img" src="{{'del_spec.png'|image}}" title="删除规格" />
                                    {{$spec_name}}
                                </dt>
                                <dd class="cc">
                                {{foreach $spec_val as $s_val}}
                                    <div class="spec_item">
                                        <span class="spec_cbx"> <input type="checkbox" checked="">
                                        </span> <label class="spec_txt"> <span style="display: none;">{{$s_val}}</span>
                                            <input style="display: inline-block;" type="text" name="spec_val_alias[{{$spec_name}}:::{{$s_val}}]" class="form-input" data-spec-val="{{$s_val}}" value="{{$s_val}}">
                                        </label>
                                    </div>
                                {{/foreach}}
                                    <div class="spec_item">
                                        <span class="spec_cbx">
                                        <input type="checkbox">
                                        </span><label class="spec_txt">
                                        <span>其他</span>
                                        <input style="display: none;" disabled="" type="text" name="spec_val_alias[{{$spec_name}}:::{{''|uniqid}}]" class="form-input" data-spec-val="{{''|uniqid}}" value="其他">
                                        </label>
                                    </div>
                                </dd>
                            </dl>
                            ...
                            <div id="goods-prods-table">
                                ...
                                <table class="prods-table m-t-10">
                                    ...
                                    <tbody>
                                        {{foreach $data.products as $pro_spec_val => $pro}}
                                        <tr>
                                            {{if !empty($data.spec_val) }}
                                            {{foreach $data.spec_val as $spec_name => $spec_val}}
                                            <td>{{$pro.arr_spec_val.$spec_name}}</td>
                                            {{/foreach}}
                                            {{/if}}
                                            <td><input type="text" class="form-input" name="products[{{$pro.spec_val}}][market_price]" style="width:55px;" value="{{$pro.market_price}}" placeholder="￥"> 金币</td>
                                            <td><input type="text" class="form-input" name="products[{{$pro.spec_val}}][sales_price]" style="width:55px;" value="{{$pro.sales_price}}" placeholder="￥"> 金币</td>
                                            <td><input type="text" class="form-input" name="products[{{$pro.spec_val}}][product_stock]" style="width:55px;" value="{{$pro.stock}}" placeholder="999"> </td>
                                            <td><input type="text" class="form-input" name="products[{{$pro.spec_val}}][sku_id]" placeholder="货品 SKU" value="{{$pro.skuid}}" style="width:100px;"></td>
                                        </tr>
                                        {{/foreach}}
                                    </tbody>
                                </table>
                                ...
                                <div id="single-prods-form">
                                    <table class="prods-table m-t-10">
                                        ...
                                        <tbody>
                                            <tr>
                                                <td><input type="text" class="form-input" name="products[single_product][market_price]" style="width:50px;" value="{{$data.products.single_product.market_price}}" placeholder="￥"> 金币</td>
                                                <td><input type="text" class="form-input" name="products[single_product][sales_price]" style="width:50px;" value="{{$data.products.single_product.sales_price}}" placeholder="￥"> 金币</td>
                                                <td><input type="text" class="form-input" name="products[single_product][product_stock]" style="width:50px;" value="{{$data.products.single_product.stock}}" placeholder="999"> </td>
                                                <td><input type="text" class="form-input" name="products[single_product][sku_id]" placeholder="SKU" value="{{$data.products.single_product.skuid}}" style="width:100px;"></td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </div>
            ...
            <tr>
                <th class="left-txt">商品详情(g)：</th>
                <td><textarea name="description" id="editor_id" style="width: 90%; height: 400px;" rows="5" cols="50">{{$data.description}}</textarea></td>
            </tr>
            <tr>
				<td></td>
				<td>
                    <input type="hidden" name="goods_id" id="goods_id" value="{{$data.goodsid}}" />
        ...
        function getProdTableTpl(selectedSpecInfo, cacheProdsTableData) {
            if (selectedSpecInfo.ids.length == 0) {
                ...
                var trBase = function (name) {
                    ...
                    var
                    priceTdHtml  = '<td><input type="text" class="form-input" name="products['+name+ '][market_price]" style="width:50px;" value="' + market_price + '" placeholder="￥"/> 元</td>';
                    priceTdHtml += '<td><input type="text" class="form-input" name="products['+name+ '][sales_price]" style="width:50px;" value="' + sales_price + '" placeholder="￥"/> 元</td>';
