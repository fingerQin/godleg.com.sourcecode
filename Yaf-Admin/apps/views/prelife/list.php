		...
		<div class="search-box row">
			<div class="col-md-12">
				<form action="{{'Prelife/list'|url}}" method="get">
                    ...
                    <div class="form-group">
                        <span class="pull-left form-span">身份类型</span>
						<select id="type" name="type" class="form-control" style="width:100px;">
                            <option value="-1">请选择</option>
							<option {{if $type==1}}selected="selected"{{/if}} value="1">士</option>
                            <option {{if $type==2}}selected="selected"{{/if}} value="2">农</option>
							<option {{if $type==3}}selected="selected"{{/if}} value="3">工</option>
							<option {{if $type==4}}selected="selected"{{/if}} value="4">商</option>
						</select>
					</div>
					<div class="form-group">
				...
				<tbody>
    				...
    				<tr>
						...
						<td class="text-center">
							{{if 'Prelife'|access:'edit'}}
							<a href="###" onclick="edit({{$item.id}}, '{{$item.title}}')" title="修改">修改</a> | 
							{{/if}}
							{{if 'Prelife'|access:'delete'}}
							<a href="###" onclick="deleteDialog('deletePrelife', '{{'Prelife'|url:delete:['id' => $item.id]}}', '{{$item.title}}')" title="删除">删除</a>
							{{/if}}
						</td>
					</tr>
    				{{/foreach}}
...
function add() {
	postDialog('addName', '{{'Prelife/add'|url}}', '添加游戏配置', 350, 330);
}
function edit(id, name) {
	var title = '修改『' + name + '』';
	var page_url = "{{'Prelife/edit'|url}}?id="+id;
