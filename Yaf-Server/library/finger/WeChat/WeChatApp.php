...
namespace finger\WeChat;

use Utils\YCore;
use finger\WeChat\Pay\WxPayConfig;
use Utils\YUrl;
use Common\YString;
...
class WeChatApp {
    ...
    public function getAccessToken() {
        ...
        $cache_key    = "wechat_{$appid}_access_token";
    ...
    public function getSignPackage($url = '') {
        $url         = strlen($url) > 0 ? $url : YUrl::getUrl();
        $jsapiTicket = $this->getJsApiTicket();
        $timestamp   = time();
        $nonceStr    = YString::randomstr(10);
